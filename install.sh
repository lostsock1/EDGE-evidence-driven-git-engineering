...
REPO_DIR="${RDD_REPO_DIR:-$HOME/projects/${RDD_PROJECT_NAME}}"
...
# chat, branch, or required-check identity.
cat > "$CONFIG_DIR/config.env" <<CFGEOF
# Shared dispatch policy (generated by install.sh)
RDD_DEFAULT_PROJECT_CONFIG=$CONFIG_DIR/${RDD_PROJECT_SLUG}.env
...
CFGEOF
cat > "$CONFIG_DIR/${RDD_PROJECT_SLUG}.env" <<PROJECTEOF
# Project identity: $RDD_PROJECT_NAME (generated by install.sh)
RDD_PROJECT_NAME=$RDD_PROJECT_NAME
RDD_PROJECT_SLUG=$RDD_PROJECT_SLUG
RDD_REPO_SLUG=$RDD_REPO_SLUG
